...
app_name = 'JHDICT'
username = 'admin'
token = '35b15bdc-d679-11ec-81d8-f6fee434d545'
...
def get_trans():
    r = requests.get(f'http://{address}/api/v1.0/workflows/4/init_state', headers=get_header())
    result = r.json()
    res_dict = {}
...
def get_workflow_status():
    per_page = 20000
    data = dict(per_page=per_page, category='all', act_state_id=1, workflow_ids=8)
    r = requests.get(f'http://{address}/api/v1.0/tickets', headers=get_header(), params=data)

    total = r.json().get('data').get('total')
    if total > per_page:
        data = dict(per_page=total, category='all', act_state_id=1, workflow_ids=8)
        r = requests.get(f'http://{address}/api/v1.0/tickets', headers=get_header(), params=data)
    result = r.json()
    return result
...
def check_all_id(id_list):
    ...
    for data_dict in result['data']['value']:
        ...
        for data_dict2 in tk_val['data']['value']['field_list']:
            if data_dict2['field_key'] == 'project_code':
